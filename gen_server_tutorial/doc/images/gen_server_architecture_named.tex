...
\node[font=\small\ttfamily\bfseries\itshape\color{black}, anchor=north west] at ([xshift=3pt, yshift=-1pt]rightframe.north west) {gen\_server.erl};
    ...
    \node[draw=erlred!70, fill=erlred!8, rounded corners=3pt,
          font=\scriptsize\ttfamily\bfseries, inner sep=3pt, anchor=north west, align=left]
        (startlink) at ([xshift=6pt, yshift=-42pt]lt.north west)
        {\textcolor{erlred!80!black}{\ding{72} gen\_server:start\_link(}\\
         \hspace{1cm}\textcolor{erlred!80!black}{\textcolor{erlname}{\textbf{ServerName}}, \textcolor{erlmodule}{\textbf{?MODULE}}, Args, Opts)}};
...
\textcolor{erlinit}{\textbf{init/1}}\\
    ...
    \node[font=\scriptsize\ttfamily, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-18pt]lb.north west) {
        gen\_server:call(\textcolor{erlname}{\textbf{ServerRef}}, Req)\\
        gen\_server:cast(\textcolor{erlname}{\textbf{ServerRef}}, Req)\\
        gen\_server:send\_request(\textcolor{erlname}{\textbf{ServerRef}}, Req)\\
        gen\_server:stop(\textcolor{erlname}{\textbf{ServerRef}})
    };
    % Note: ServerRef = Name | {Name,Node} | {global,GlobalName}
    \node[font=\tiny\color{gray}, anchor=south west] at ([xshift=6pt, yshift=3pt]lb.south west)
        {备注：\textcolor{erlname}{\textbf{ServerRef}} = \texttt{Name} | \texttt{\{Name,Node\}} | \texttt{\{global,GlobalName\}}};
    ...
    \node[font=\scriptsize, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rt.north west) {
        ...
        {\tiny\quad$\rightarrow$ \textcolor{erlname}{\textbf{\texttt{register(ServerName, Pid)}}} $\rightarrow$ \textcolor{erlinit}{\textbf{\texttt{Mod:init/1}}}}\\[6pt]
        \textcolor{erlblue!80!black}{\ding{72} \textbf{\texttt{\#server\_data\{\}} 记录：}}\\
        {\tiny\makebox[0pt][l]{\texttt{parent\!=\!}父进程Pid,\;\texttt{module\!=\!}\textcolor{erlmodule}{\textbf{\texttt{?MODULE}}},\;\textcolor{erlname}{\textbf{name}}\!\texttt{=}\!\textcolor{erlname}{ServerName}}}\\
    ...
    \node[font=\scriptsize, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rb.north west) {
        \textcolor{erlred!80!black}{\texttt{start\_link(\textcolor{erlname}{\textbf{ServerName}}, \textcolor{erlmodule}{\textbf{?MODULE}}, ...)}} 之后：\\[3pt]
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlred!80!black, line width=1.6pt]
        ...
        {\textcolor{black}{\ding{172}} 启动服务 $\rightarrow$ 传递\textcolor{erlname}{\textbf{名字}}+模块名\\[1pt]
         \texttt{start\_link(\textcolor{erlname}{\textbf{ServerName}},}\\[1pt]
         \texttt{\hspace{2cm}\textcolor{erlmodule}{\textbf{?MODULE}}, Args, Opts)}} ([yshift=3.8mm]rt.west);
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlpid!90!black, line width=1.6pt]
        ...
        {\textcolor{black}{\ding{174}} 返回 \texttt{\textcolor{erlpid}{\textbf{\{ok, Pid\}}}}（进程已\textcolor{erlname}{\textbf{注册名字}}）\\[1pt]
         {\tiny \textcolor{erlinit}{\textbf{init/1}} 成功后通过 \texttt{init\_ack} 返回 Pid，}\\[1pt]
         {\tiny 进程已注册为 \textcolor{erlname}{\textbf{ServerName}}}} ([yshift=-6.5mm]lt.east);
    ...
    \node[font=\tiny\color{erlred!70!black}, anchor=north west, text width=5.8cm, align=left] at ([xshift=6pt, yshift=-7pt]lb.south west)
        {\ding{72} \texttt{start\_link/4} 是连接左右两半的关键桥梁：\\
         \ding{72} 这里 \texttt{\textcolor{erlmodule}{\textbf{?MODULE}}} 是宏，指的是 \texttt{counter\_server}\\
         \ding{72} \textcolor{erlname}{\textbf{ServerName}} 使进程注册到本地/全局名字表\\
         \ding{72} Client 可通过 \textcolor{erlname}{\textbf{ServerName}} 而非 Pid 调用服务\\
         \ding{72} 当 \textcolor{erlname}{\textbf{ServerRef}} 是全局名字表时，即是分布式通信};

    % Right bottom annotation (inside right outer frame, below dashed line)
    \node[font=\tiny\color{erlred!70!black}, anchor=north west, text width=5.8cm, align=left] at ([xshift=6pt, yshift=-8pt]rb.south west)
        {\ding{72} \textcolor{erlinit}{\textbf{\texttt{Mod:init/1}}} 启动时即调用用户编写的 \textcolor{erlinit}{\textbf{\texttt{init/1}}}\\
         ...
         \ding{72} 启动成功后返回 \textcolor{erlpid}{\textbf{\texttt{\{ok, Pid\}}}}，进程同时注册为 \textcolor{erlname}{\textbf{ServerName}}\\
         \ding{72} Client 可通过 \textcolor{erlname}{\textbf{ServerName}}（而非 Pid）调用 \texttt{call/cast} 等};
