...
\textcolor{erlinit}{\textbf{init/1}}\\
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlred!80!black, line width=1.6pt, densely dashed]
        ([yshift=5mm]lt.east) -- node[above, font=\scriptsize, align=center, text=erlred!80!black]
        {\textcolor{black}{\ding{172}} 传递模块名 $\rightarrow$ 绑定回调函数\\[1pt]
         \texttt{start\_link(\textcolor{erlmodule}{\textbf{?MODULE}}, Args, Opts)}} ([yshift=5mm]rt.west);
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlpid!90!black, line width=1.6pt]
        ([yshift=-1mm]rt.west) -- node[below, font=\scriptsize, align=center, text=erlpid!80!black]
        {\textcolor{black}{\ding{172}'} 返回 \texttt{\textcolor{erlpid}{\textbf{\{ok, Pid\}}}}\\[1pt]
         {\tiny \textcolor{erlinit}{\textbf{init/1}} 成功后通过 \texttt{init\_ack} 返回 Pid}} ([yshift=-1mm]lt.east);

    % Arrow ②: Function registration — Callback source (lt) -> Bound Callbacks (via ?MODULE)
    \draw[-{Stealth[length=6pt]}, thick, erlgreen!70!black, line width=1.2pt, dashed]
        ([yshift=9.5mm]lb.east) -- node[api, above, align=center, text width=3cm, text=erlgreen!70!black]
        {\scriptsize\bfseries \textcolor{black}{\ding{173}} 函数注册\\[1pt]
         \tiny \texttt{\textcolor{erlmodule}{\textbf{?MODULE}}} $\rightarrow$ \texttt{fun Mod:F/A}} ([yshift=9.5mm]rb.west);

    % ==== Phase 2: Runtime ====

    % Arrow ③: Client API (lb) -> Generic Server (call / cast / send_request)
    \draw[-{Stealth[length=5pt]}, thick, erlblue, line width=1.2pt]
        ([yshift=-4mm]lb.east) -- node[api, above, align=center] {\textcolor{black}{\ding{174}} \textcolor{erlblue}{call / cast / send\_request}} ([yshift=-4mm]rb.west);
    ...
    % Arrow ⑥: Generic Server -> Client API (Reply) — to lb
    \draw[-{Stealth[length=5pt]}, thick, erlred, line width=1.2pt]
        ([yshift=-14mm]rb.west) -- node[api, below] {\textcolor{black}{\ding{177}} \textcolor{erlred}{Reply}} ([yshift=-14mm]lb.east);
