...
\textcolor{erlinit}{\textbf{init/1}}\\
    ...
    \node[font=\scriptsize, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rt.north west) {
        ...
        {\tiny\texttt{start\_link} $\rightarrow$ \texttt{proc\_lib:start\_link} $\rightarrow$ \texttt{init\_it} $\rightarrow$ \textcolor{erlinit}{\textbf{\texttt{Mod:init/1}}}}\\[22pt]
        ...
        {\tiny\texttt{receive} $\rightarrow$ \texttt{Mod:handle\_call/3} \ldots}
    };
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlred!80!black, line width=1.6pt, densely dashed]
        ...
        {\textcolor{black}{\ding{172}} 传递模块名 $\rightarrow$ 绑定回调函数\\[1pt]
    ...
    \draw[-{Stealth[length=5pt]}, thick, erlblue, line width=1.2pt]
        ([yshift=-4mm]lt.east) -- node[api, above, align=center] {\textcolor{black}{\ding{174}} \textcolor{erlblue}{call / cast / send\_request}} ([yshift=-4mm]rt.west);

    % Arrow ④: Generic Server -> Bound Callbacks (callback invocation)
    \draw[-{Stealth[length=5pt]}, thick, erlblue!70, line width=1pt]
        ([xshift=-5mm]rt.south) -- node[left, font=\scriptsize, text=erlblue!70] {\textcolor{black}{\ding{175}} callback} ([xshift=-5mm]rb.north);

    % Arrow ⑤: Bound Callbacks -> Generic Server (return value)
    \draw[-{Stealth[length=5pt]}, thick, erlgreen!70, line width=1pt]
        ([xshift=5mm]rb.north) -- node[right, font=\scriptsize, text=erlgreen!70] {\textcolor{black}{\ding{176}} return value} ([xshift=5mm]rt.south);

    % Arrow ⑥: Generic Server -> Client API (Reply)
    \draw[-{Stealth[length=5pt]}, thick, erlred, line width=1.2pt]
        ([yshift=-14mm]rt.west) -- node[api, below] {\textcolor{black}{\ding{177}} \textcolor{erlred}{Reply}} ([yshift=-14mm]lt.east);
