...
\begin{figure}[H]
\centering
% Architecture diagram is maintained in images/gen_server_architecture.tex
% Build it separately: cd images && make
\includegraphics[width=\textwidth]{images/gen_server_architecture.pdf}
...
\begin{itemize}[leftmargin=2em, itemsep=2pt]
    ...
    \item \ding{173} \textbf{函数注册}（虚线）：左下角用户编写的回调函数（\texttt{counter\_server.erl}），通过 \texttt{?MODULE} 传递，
          在右下角变为 \texttt{fun counter\_server:handle\_call/3} 等函数引用——\textbf{代码在左边写，在右边执行}
    \item[\textbf{运行阶段}]~
    \item \ding{174} \textbf{API 调用}：Client 通过 \texttt{gen\_server:call/cast} 向 Server 发送消息
    \item \ding{175} \textbf{回调}：Generic Server 收到消息后，通过缓存的函数引用调用已绑定的回调函数
    \item \ding{176} \textbf{返回值}：Callback Module 通过 \texttt{\{reply, Reply, NewState\}} 等元组返回结果
    \item \ding{177} \textbf{回复}：Generic Server 将处理结果返回给 Client
\end{itemize}
\end{notebox}
