...
\texttt{gen\_server\_tutorial.tex} 从零开始，经历了"纯 Erlang 进程 $\to$ 自定义通用服务器 $\to$ OTP gen\_server"的演进过程，重点在于\textbf{理解原理}。

本文档则完全不同：我们\textbf{直接使用 gen\_server 库函数}，通过一个最小的回调模块，在 Erlang Shell 中交互式地演示 gen\_server 的每一个 API。

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    doc/.style={draw=#1, fill=#1!8, rounded corners=8pt,
                minimum width=5.5cm, minimum height=2.5cm, text width=5cm, align=center},
]
    \node[doc=erlblue] (tut) at (0, 0) {
        \textbf{\textsf{gen\_server\_tutorial.tex}}\\[4pt]
        \small 原理演进\\
        \small 从零构建\\
        \small 理解 Behaviour 本质
    };
    \node[doc=erlred] (api) at (8, 0) {
        \textbf{\textsf{gen\_server\_api\_guide.tex}}\\[4pt]
        \small 直接使用库函数\\
        \small Shell 交互演示\\
        \small API 全覆盖
    };
    \draw[-{Stealth[length=6pt]}, thick, erlpurple, dashed]
        (tut.east) -- node[above, font=\small\itshape] {互补} (api.west);
\end{tikzpicture}
\caption{两份文档的定位对比}
\label{fig:doc-positioning}
\end{figure}
...
7 & 不要在 \texttt{handle\_call} 中调用 \texttt{self()} \\
