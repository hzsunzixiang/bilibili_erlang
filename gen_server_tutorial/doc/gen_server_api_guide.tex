...
\begin{tikzpicture}[
    ...
    \node[draw=erlpurple!60, fill=erlpurple!3, rounded corners=6pt,
          minimum width=6.8cm, minimum height=10cm] (leftframe) at (-3.8, 0) {};
    ...
    \node[draw=erlblue!60, fill=erlblue!3, rounded corners=6pt,
          minimum width=6.8cm, minimum height=10cm] (rightframe) at (3.8, 0) {};
    ...
    \node[draw=erlpurple, fill=erlpurple!8, rounded corners=5pt,
          minimum width=6cm, minimum height=4cm, text width=5.5cm, align=left] (lt) at (-3.8, 2.3) {};
    \node[font=\bfseries\color{erlpurple}, anchor=north west] at ([xshift=3pt, yshift=-3pt]lt.north west) {Client API};
    \node[font=\small\ttfamily, text width=5.2cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-18pt]lt.north west) {
        gen\_server:start\_link/3,4\\
    ...
    \node[draw=gray, fill=gray!5, rounded corners=5pt,
          minimum width=6cm, minimum height=4cm, text width=5.5cm, align=left] (lb) at (-3.8, -2.3) {};
    \node[font=\bfseries\color{gray!80!black}, anchor=north west] at ([xshift=3pt, yshift=-3pt]lb.north west) {User / Developer};
    \node[font=\small, text width=5.2cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-18pt]lb.north west) {
        编写 Callback Module\\
        定义业务逻辑\\
        通过 \texttt{start\_link(Mod, ...)} 将\\
        Module 注册到 gen\_server 框架
    };

    % ---- Right-Top: Generic Server ----
    \node[draw=erlblue, fill=erlblue!10, rounded corners=5pt,
          minimum width=6cm, minimum height=4cm, text width=5.5cm, align=left] (rt) at (3.8, 2.3) {};
    \node[font=\bfseries\color{erlblue}, anchor=north west] at ([xshift=3pt, yshift=-3pt]rt.north west) {Generic Server (gen\_server.erl)};
    \node[font=\small, text width=5.2cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-18pt]rt.north west) {
        消息循环 (\texttt{loop/4})\\
        消息路由与分发\\
        超时 / 休眠处理\\
        错误处理 \& crash report\\
    ...
    \node[draw=erlgreen, fill=erlgreen!8, rounded corners=5pt,
          minimum width=6cm, minimum height=4cm, text width=5.5cm, align=left] (rb) at (3.8, -2.3) {};
    \node[font=\bfseries\color{erlgreen}, anchor=north west] at ([xshift=3pt, yshift=-3pt]rb.north west) {Callback Module (Your Code)};
    \node[font=\small\ttfamily, text width=5.2cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-18pt]rb.north west) {
        init/1\\
        handle\_call/3\\
        handle\_cast/2\\
        handle\_info/2\\
    ...
    % ---- Arrows (numbered for cross-reference with Section 1.3) ----
    % Arrow 1: Client API -> Generic Server (API calls)
    \draw[-{Stealth[length=6pt]}, thick, erlblue, line width=1.5pt]
        ([yshift=8mm]lt.east) -- node[api, above] {\textcolor{erlblue}{\ding{182} call / cast / send\_request}} ([yshift=8mm]rt.west);

    % Arrow 2: Generic Server -> Client API (Reply)
    \draw[-{Stealth[length=6pt]}, thick, erlred, line width=1.5pt]
        ([yshift=-4mm]rt.west) -- node[api, below] {\textcolor{erlred}{\ding{183} Reply}} ([yshift=-4mm]lt.east);

    % Arrow 3: Generic Server -> Callback Module (callback invocation)
    \draw[-{Stealth[length=6pt]}, thick, erlblue!70, line width=1.2pt]
        ([xshift=-15mm]rt.south) -- node[left, font=\scriptsize\itshape, text=erlblue!70] {\ding{184} callback} ([xshift=-15mm]rb.north);

    % Arrow 4: Callback Module -> Generic Server (return value)
    \draw[-{Stealth[length=6pt]}, thick, erlgreen!70, line width=1.2pt]
        ([xshift=15mm]rb.north) -- node[right, font=\scriptsize\itshape, text=erlgreen!70] {\ding{185} return value} ([xshift=15mm]rt.south);

    % Arrow 0: User/Developer -> Callback Module (function registration, happens at start_link time)
    \draw[-{Stealth[length=6pt]}, thick, erlpurple, line width=1.2pt, dashed]
        (lb.east) -- node[api, above] {\textcolor{erlpurple}{\ding{172} 函数注册：Module 作为参数传递}} (rb.west);
    \node[font=\scriptsize\color{erlpurple!70}, anchor=north] at (0, -4.6) {函数式编程：行为的延迟绑定（详见 \S\ref{subsec:arch-annotation}）};
...
\begin{itemize}[leftmargin=2em, itemsep=2pt]
    \item \ding{172} \textbf{函数注册}（虚线）：\texttt{start\_link} 时，Module 名（原子）作为参数传入 Generic Server，绑定回调函数
    ...
    \item \ding{184} \textbf{回调}：Generic Server 收到消息后，调用 Callback Module 中对应的回调函数
...
\midrule
Phase 1: \texttt{start\_link} & \texttt{counter\_server} 作为参数传入，被存入 \texttt{\#server\_data.module}， & \checkmark \\
 & 同时 \texttt{fun counter\_server:handle\_call/3} 等函数引用被缓存 & \\
\midrule
Phase 2: \texttt{call} & 客户端只向 Pid 发送 \texttt{\{'\$gen\_call', From, decrement\}} 消息 & --- \\
\midrule
Phase 3: dispatch & 框架从 \texttt{\#server\_data} 中取出缓存的 \texttt{HandleCall} & \checkmark \\
 & 函数引用，直接调用 \texttt{counter\_server:handle\_call/3} & (from cache) \\
\bottomrule
\end{tabular}
\caption{三个阶段中模块名的角色}
...
\begin{tikzpicture}[
    ...
    \draw[thick, erlred!50] (10, 0) -- (10, -7) node[below, font=\small] {Server};

    % call
    \draw[-{Stealth[length=5pt]}, thick, erlblue]
        (0, -0.5) -- node[above, font=\scriptsize\ttfamily] {call(Pid, \{slow\_op, 3000\}, 1000)} (10, -0.5);

    % timeout line
    \draw[dashed, erlred, thick] (-1, -2) -- (5, -2);
    \node[font=\scriptsize\bfseries\color{erlred}, anchor=east] at (-1.2, -2) {1000ms timeout!};
    ...
    \draw[thick, erlgreen, decorate, decoration={brace, amplitude=5pt, mirror}]
        (10.3, -0.5) -- (10.3, -4.5)
        node[midway, right=8pt, font=\scriptsize, text width=3cm] {Server still processing... (3000ms sleep)};

    % Late reply
    \draw[-{Stealth[length=5pt]}, thick, gray, dashed]
        (10, -5) -- node[above, font=\scriptsize\ttfamily\color{gray}] {late reply (discarded)} (0, -5);

    \node[font=\scriptsize\color{gray}] at (5, -5.8) {Reply arrives at dead/restarted client};
\end{tikzpicture}
...
\begin{tikzpicture}[
    ...
    \draw[thick, erlred!50] (8, 0) -- (8, -8) node[below, font=\small] {Server1};
    \draw[thick, erlgreen!50] (11, 0) -- (11, -8) node[below, font=\small] {Server2};
    \draw[thick, erlpurple!50] (14, 0) -- (14, -8) node[below, font=\small] {Server3};

    % Send all requests (non-blocking)
    \draw[-{Stealth[length=4pt]}, thick, erlred]
        (0, -0.5) -- node[above, font=\tiny\ttfamily] {send\_request} (8, -0.8);
    \draw[-{Stealth[length=4pt]}, thick, erlgreen]
        (0, -1.0) -- node[above, font=\tiny\ttfamily] {send\_request} (11, -1.5);
    \draw[-{Stealth[length=4pt]}, thick, erlpurple]
        (0, -1.5) -- node[above, font=\tiny\ttfamily] {send\_request} (14, -2.2);
    ...
    % Replies come back
    \draw[-{Stealth[length=4pt]}, thick, erlgreen]
        (11, -4) -- node[above, font=\tiny\ttfamily] {reply} (0, -4.5);
    \draw[-{Stealth[length=4pt]}, thick, erlred]
        (8, -4.5) -- node[above, font=\tiny\ttfamily] {reply} (0, -5.2);
    \draw[-{Stealth[length=4pt]}, thick, erlpurple]
        (14, -5.5) -- node[above, font=\tiny\ttfamily] {reply} (0, -6.2);
...
\begin{tikzpicture}[
    reg/.style={draw=#1, fill=#1!8, rounded corners=6pt,
                minimum width=5cm, minimum height=2.5cm, text width=4.5cm, align=center},
]
