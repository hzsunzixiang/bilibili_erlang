...
\begin{tikzpicture}[scale=0.72, every node/.style={scale=0.72},
    ...
    % ---- Outer frames ----
    % Left half (moved further left for more gap)
    \node[draw=erlpurple!60, fill=erlpurple!3, rounded corners=6pt,
          minimum width=6.8cm, minimum height=10cm] (leftframe) at (-5.2, 0) {};
    \node[font=\small\bfseries\color{erlpurple}, anchor=south] at (leftframe.north) {调用方 / 外部};

    % Right half (moved further right for more gap)
    \node[draw=erlblue!60, fill=erlblue!3, rounded corners=6pt,
          minimum width=6.8cm, minimum height=10cm] (rightframe) at (5.2, 0) {};
    ...
    \node[draw=erlpurple, fill=erlpurple!8, rounded corners=5pt,
          minimum width=6cm, minimum height=4cm, text width=5.5cm, align=left] (lt) at (-5.2, 2.3) {};
    \node[font=\small\bfseries\color{erlpurple}, anchor=north west] at ([xshift=3pt, yshift=-3pt]lt.north west) {Client API};
    \node[font=\scriptsize\ttfamily, text width=5.2cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]lt.north west) {
    ...
    \node[draw=gray, fill=gray!5, rounded corners=5pt,
          minimum width=6cm, minimum height=4cm, text width=5.5cm, align=left] (lb) at (-5.2, -2.3) {};
    \node[font=\small\bfseries\color{gray!80!black}, anchor=north west] at ([xshift=3pt, yshift=-3pt]lb.north west) {User / Developer};
    \node[font=\scriptsize, text width=5.2cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]lb.north west) {
        编写 Callback Module\\
        定义业务逻辑\\
    ...
    \node[draw=erlblue, fill=erlblue!10, rounded corners=5pt,
          minimum width=6cm, minimum height=4cm, text width=5.5cm, align=left] (rt) at (5.2, 2.3) {};
    ...
    \node[font=\scriptsize, text width=5.2cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rt.north west) {
        消息循环 (\texttt{loop/4})\\
        消息路由与分发\\
        超时 / 休眠处理\\
        错误处理 \& crash report\\
    ...
    \node[draw=erlgreen, fill=erlgreen!8, rounded corners=5pt,
          minimum width=6cm, minimum height=4cm, text width=5.5cm, align=left] (rb) at (5.2, -2.3) {};
    ...
    \node[font=\scriptsize\ttfamily, text width=5.2cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rb.north west) {
        init/1\\
        handle\_call/3\\
        handle\_cast/2\\
        handle\_info/2\\
    ...
    % Arrow 1: Client API -> Generic Server (API calls) — label with line break
    \draw[-{Stealth[length=5pt]}, thick, erlblue, line width=1.2pt]
        ([yshift=8mm]lt.east) -- node[api, above, align=center, text width=2.2cm] {\textcolor{erlblue}{\ding{182} call / cast /\\send\_request}} ([yshift=8mm]rt.west);
    ...
    % Arrow 0: User/Developer -> Callback Module (function registration) — label with line break
    \draw[-{Stealth[length=5pt]}, thick, erlpurple, line width=1pt, dashed]
        (lb.east) -- node[api, above, align=center, text width=2.8cm] {\textcolor{erlpurple}{\ding{172} 函数注册：\\Module 作为\\参数传递}} (rb.west);
    \node[font=\tiny\color{erlpurple!70}, anchor=north] at (0, -4.8) {函数式编程：行为的延迟绑定（详见 \S\ref{subsec:arch-annotation}）};
