...
下图展示了 gen\_server 的完整架构，包括 Client 如何通过 API 调用 Server，以及 Server 内部 Generic Part 与 Callback Module 之间的交互关系。
这一架构直接对应 OTP 源码 \texttt{gen\_server.erl} 中的注释：

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    block/.style={draw, rounded corners=4pt, minimum width=8.5cm, text width=8cm, align=left, font=\small\ttfamily},
    api/.style={font=\scriptsize\ttfamily, inner sep=2pt},
]
    % ---- Client (left side) ----
    \node[draw=erlpurple, fill=erlpurple!8, rounded corners=6pt,
          minimum width=3.2cm, minimum height=3cm] (client) at (-5.5, 0) {};
    \node[font=\large\bfseries\color{erlpurple}, anchor=north] at (client.north) {Client Process};
    \node[font=\small, text width=3cm, align=center] at (-5.5, -0.5) {
        Any Erlang process\\
        that calls the\\
        gen\_server API
    };
    ...
    % ---- Server outer frame ----
    \node[draw=erlblue, fill=erlblue!3, rounded corners=8pt,
          minimum width=10cm, minimum height=11.5cm] (outer) at (4, -0.5) {};
    \node[font=\Large\bfseries\color{erlblue}, anchor=north] at (outer.north) {gen\_server Process};

    % ---- Generic Part (top half of server) ----
    \node[block, fill=erlblue!12, draw=erlblue!60,
          minimum height=4cm] (generic) at (4, 2) {
        \textbf{\textsf{Generic Part (gen\_server.erl)}} \\[4pt]
        - Process spawning \& registration \\
        - Message receiving loop (\texttt{loop/4}) \\
        - Timeout / hibernate handling \\
        - Error handling \& crash reports \\
        - sys module integration \\
        - Hot code upgrade support
    };

    % ---- Specific Part (bottom half of server) ----
    \node[block, fill=erlgreen!12, draw=erlgreen!60,
          minimum height=4cm] (specific) at (4, -3.5) {
        \textbf{\textsf{Callback Module (Your Code)}} \\[4pt]
        ...
        - handle\_continue/2 (continuation) \\
        - terminate/2 \quad\quad\;\; (cleanup) \\
        - code\_change/3 \quad\; (hot upgrade)
    };

    % ---- Callback arrows (Generic <-> Specific) ----
    \draw[-{Stealth[length=6pt]}, thick, erlblue!70, line width=1.2pt]
        ([xshift=-1.5cm]generic.south) -- node[left, font=\scriptsize\itshape, text=erlblue!70] {callback} ([xshift=-1.5cm]specific.north);
    \draw[-{Stealth[length=6pt]}, thick, erlgreen!70, line width=1.2pt]
        ([xshift=1.5cm]specific.north) -- node[right, font=\scriptsize\itshape, text=erlgreen!70] {return value} ([xshift=1.5cm]generic.south);

    % ---- Client -> Server API arrows ----
    % All arrows are horizontal and evenly spaced vertically
    % Y positions: 3.0, 1.8, 0.6, -0.6, -1.8 (evenly spaced by 1.2)

    % 1. call (sync) -->
    \draw[-{Stealth[length=6pt]}, thick, erlblue, line width=1.5pt]
        (-3.8, 3.0) -- node[api, above] {\textcolor{erlblue}{gen\_server:call/2,3}} (-0.6, 3.0);
    \node[api, anchor=east, text=erlblue] at (-4.0, 3.0) {sync (blocks)};

    % 2. reply <--
    \draw[{Stealth[length=6pt]}-, thick, erlred, line width=1.5pt]
        (-3.8, 1.8) -- node[api, above] {\textcolor{erlred}{Reply}} (-0.6, 1.8);

    % 3. cast (async) -->
    \draw[-{Stealth[length=6pt]}, thick, erlgreen!80!black, line width=1.5pt]
        (-3.8, 0.6) -- node[api, above] {\textcolor{erlgreen!80!black}{gen\_server:cast/2}} (-0.6, 0.6);
    \node[api, anchor=east, text=erlgreen!80!black] at (-4.0, 0.6) {async (fire \& forget)};

    % 4. start_link (dashed) -->
    \draw[-{Stealth[length=6pt]}, thick, erlpurple, line width=1.2pt, dashed]
        (-3.8, -0.6) -- node[api, above] {\textcolor{erlpurple}{gen\_server:start\_link/3,4}} (-0.6, -0.6);

    % 5. stop (dashed) -->
    \draw[-{Stealth[length=6pt]}, thick, gray, line width=1.2pt, dashed]
        (-3.8, -1.8) -- node[api, above] {\textcolor{gray}{gen\_server:stop/1,3}} (-0.6, -1.8);

\end{tikzpicture}
\caption{gen\_server 完整架构：Client 调用方向与 Callback 机制}
\label{fig:architecture}
\end{figure}

对应 OTP 源码中的工作流注释（摘自 \texttt{gen\_server.erl}）：
