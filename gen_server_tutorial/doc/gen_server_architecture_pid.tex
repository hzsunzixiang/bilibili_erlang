...
下面两张架构图分别展示了 gen\_server 的两种典型使用方式：通过 PID 引用和通过注册名字引用。

\subsection{方式一：通过 PID 引用（start\_link/3）}
...
\subsubsection{实验演示：分布式 PID 场景（跨节点访问）}
...
\subsection{方式二：通过注册名字引用（start\_link/4）--- 进程注册方式}
...
\subsubsection{server\_ref() 完整类型}
...
\subsection{陷阱 1：在 Shell 中使用 start\_link}

\begin{lstlisting}[style=erlangstyle]
%% Shell process and gen_server are linked!
1> {ok, Pid} = counter_server:start_link().
{ok,<0.83.0>}
2> counter_server:crash(Pid).
** exception exit: ...
3> self().
<0.88.0>   %% Shell process has changed!
\end{lstlisting}

Shell 进程与 gen\_server 链接，gen\_server 崩溃会导致 Shell 也崩溃（然后自动重启）。开发测试时建议使用 \texttt{start/0}。

\subsection{陷阱 2：在 handle\_call 中调用自己}
...
gen\_server 是单进程的，在处理消息时无法再接收新消息。

\subsection{陷阱 3：call 超时后的残留消息}

\begin{lstlisting}[style=erlangstyle]
%% After timeout, server's reply may still arrive later
try
...
超时时\textbf{客户端崩溃}，但\textbf{服务器继续运行}并可能稍后发送回复。

\subsection{陷阱 4：handle\_call 中的长时间操作}

\begin{lstlisting}[style=erlangstyle]
%% BAD: blocks the gen_server for Duration ms
handle_call({slow_op, Duration}, _From, State) ->
    timer:sleep(Duration),  %% All other requests are blocked!
    {reply, ok, State}.
\end{lstlisting}

gen\_server 是顺序处理消息的，长时间操作会阻塞所有其他请求。解决方案：
\begin{itemize}[leftmargin=2em]
    \item 使用 \texttt{cast} + 异步通知
    \item spawn 子进程处理耗时任务
    \item 使用 \texttt{gen\_server:reply/2} 先回复再处理
\end{itemize}
...
gen\_server 内置了 \texttt{sys} 模块集成，提供强大的调试能力：

\begin{lstlisting}[style=erlangstyle, caption={sys 模块调试工具}]
%% Get server status
sys:get_status(Pid).

%% Trace messages (print all incoming messages)
sys:trace(Pid, true).
sys:trace(Pid, false).  %% stop tracing

%% Get statistics
sys:statistics(Pid, true).   %% start collecting
...
\midrule
\texttt{call(Ref, Req)} & 同步 & 默认5秒超时 \\
\texttt{call(Ref, Req, Timeout)} & 同步 & 自定义超时 \\
\texttt{cast(Ref, Msg)} & 异步 & 无返回值 \\
\texttt{reply(From, Reply)} & --- & 显式回复 \\
\texttt{stop(Ref)} & 同步 & 停止服务器 \\
\texttt{stop(Ref, Reason, Timeout)} & 同步 & 带原因停止 \\
\texttt{send\_request(Ref, Req)} & 异步 & 异步请求（OTP 23+） \\
...
\small\ttfamily
\textbf{init/1:} \\
\quad \{ok, State\} | \{ok, State, Timeout\} | \{ok, State, \{continue, Info\}\} \\
\quad | \{stop, Reason\} | ignore \\[6pt]
\textbf{handle\_call/3:} \\
\quad \{reply, Reply, State [, Timeout | hibernate | \{continue, Info\}]\} \\
\quad \{noreply, State [, Timeout | hibernate | \{continue, Info\}]\} \\
\quad \{stop, Reason, Reply, State\} | \{stop, Reason, State\} \\[6pt]
\textbf{handle\_cast/2 \& handle\_info/2:} \\
\quad \{noreply, State [, Timeout | hibernate | \{continue, Info\}]\} \\
\quad \{stop, Reason, State\} \\[6pt]
\textbf{terminate/2:} ok (return value ignored) \\[4pt]
\textbf{code\_change/3:} \{ok, NewState\}
\end{tcolorbox}
