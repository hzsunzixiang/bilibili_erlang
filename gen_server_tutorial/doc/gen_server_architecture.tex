...
以一个计数器服务器为例，展示 gen\_server 的核心结构。模块包含以下部分：

\begin{itemize}[leftmargin=2em]
    \item \textbf{模块声明}：\texttt{-behaviour(gen\_server).} 声明行为模式，导出 Client API 和回调函数
    \item \textbf{启动函数}：
    \begin{itemize}[leftmargin=1.5em]
        \item \texttt{start\_link/0,1} --- 与调用者链接，生产环境配合 supervisor 使用
        \item \texttt{start/0,1} --- 不链接，适合开发测试（Shell 中推荐）
    \end{itemize}
    \item \textbf{客户端 API}（薄封装，运行在调用者进程中）：
    \begin{itemize}[leftmargin=1.5em]
        \item \texttt{increment/1}, \texttt{decrement/1}, \texttt{get\_count/1} --- 同步调用（\texttt{gen\_server:call}）
        \item \texttt{reset/1}, \texttt{crash/1} --- 异步调用（\texttt{gen\_server:cast}）
    \end{itemize}
    \item \textbf{init/1}：初始化 State（Map），设置 \texttt{trap\_exit}，通过 \texttt{\{continue, post\_init\}} 触发延续处理
    \item \textbf{handle\_continue/2}：处理 \texttt{init/1} 返回的 \texttt{continue} 指令，用于耗时初始化（避免阻塞 \texttt{start\_link} 调用者）
    \item \textbf{handle\_call/3}：处理同步请求，返回 \texttt{\{reply, Reply, NewState\}}
    \item \textbf{handle\_cast/2}：处理异步请求，返回 \texttt{\{noreply, NewState\}}
    \item \textbf{handle\_info/2}：处理非 call/cast 消息（如 \texttt{timeout}、\texttt{'EXIT'}、自定义消息）
    \item \textbf{terminate/2}：进程终止时的清理工作（需设置 \texttt{trap\_exit} 才能可靠触发）
    \item \textbf{format\_status/2}：自定义 \texttt{sys:get\_status/1} 输出，可隐藏敏感信息（密码、token 等）
\end{itemize}
