% supervisor Architecture Diagram — with Pid Return (Standalone)
% Based on supervisor architecture, nameless (unregistered) supervisor.
% supervisor:start_link(Module, Args) -> {ok, Pid}
% Compile with: xelatex supervisor_architecture_pid.tex
...
\begin{tikzpicture}[scale=0.68, every node/.style={scale=0.68},
    ...
    \node[font=\small\bfseries\color{erlpurple}, anchor=south] at (leftframe.north) {用户定义模块（Supervisor的启动）};
\node[font=\small\ttfamily\bfseries\itshape\color{black}, anchor=north west] at ([xshift=3pt, yshift=-1pt]leftframe.north west) {worker\_sup.erl};
    ...
    \node[font=\small\bfseries\color{erlblue}, anchor=south] at (rightframe.north) {supervisor库（监督进程引擎）};
\node[font=\small\ttfamily\bfseries\itshape\color{black}, anchor=north west] at ([xshift=3pt, yshift=-1pt]rightframe.north west) {supervisor.erl};
    \draw[dashed, black, line width=0.5pt]
    ...
    \node[font=\small\bfseries\color{erlgreen!80!black}, anchor=north west] at ([xshift=3pt, yshift=-3pt]lt.north west) {回调模块（启动supervisor）};
    \node[font=\scriptsize\ttfamily, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-18pt]lt.north west) {
        \textcolor{erlgreen!70!black}{-module(worker\_sup).}\\
        \textcolor{erlgreen!70!black}{-behaviour(supervisor).}
    };
    ...
    \node[draw=erlred!70, fill=erlred!8, rounded corners=3pt,
        ...
        {\textcolor{erlred!80!black}{\ding{72} supervisor:start\_link(\textcolor{erlmodule}{\textbf{?MODULE}}, Args)}};
    \node[font=\scriptsize\ttfamily, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-60pt]lt.north west) {
\textcolor{erlinit}{\textbf{init/1}}\\[2pt]
        {\tiny\textcolor{gray}{唯一的回调函数，返回：}}\\
        {\tiny\textcolor{gray}{\{ok, \{SupFlags, [ChildSpec]\}\}}}
    };
    ...
    \node[font=\small\bfseries\color{erlpurple}, anchor=north west] at ([xshift=3pt, yshift=-3pt]lb.north west) {Client进程（管理子进程）};
    \node[font=\scriptsize\ttfamily, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-18pt]lb.north west) {
        supervisor:start\_child(\textcolor{erlpid}{\textbf{SupRef}}, ChildSpec)\\
        supervisor:terminate\_child(\textcolor{erlpid}{\textbf{SupRef}}, Id)\\
        supervisor:which\_children(\textcolor{erlpid}{\textbf{SupRef}})\\
        supervisor:count\_children(\textcolor{erlpid}{\textbf{SupRef}})
    };
    % Note: SupRef = pid()
    \node[font=\tiny\color{gray}, anchor=south west] at ([xshift=6pt, yshift=3pt]lb.south west)
        {备注：\texttt{\textcolor{erlpid}{\textbf{SupRef}}} = \texttt{pid()}};

    % ---- Right-Top: Generic Supervisor ----
    ...
    \node[font=\small\bfseries\color{erlblue}, anchor=north west] at ([xshift=3pt, yshift=-3pt]rt.north west) {Generic Supervisor Library};
    \node[font=\scriptsize, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rt.north west) {
        ...
        {\tiny\textcolor{erlred!80!black}{\textbf{监控子进程，崩溃时按策略自动重启}}}\\
        {\tiny\texttt{receive} $\rightarrow$ \texttt{'EXIT'} $\rightarrow$ 重启子进程}\\[6pt]
        \textcolor{erlblue!80!black}{\ding{72} \textbf{启动流程：}}\\
        {\tiny\texttt{start\_link} $\rightarrow$ \texttt{proc\_lib:start\_link} $\rightarrow$ \texttt{init\_it} $\rightarrow$ \textcolor{erlinit}{\textbf{\texttt{Mod:init/1}}}}\\[6pt]
        \textcolor{erlblue!80!black}{\ding{72} \textbf{重启策略（SupFlags）：}}\\
        {\tiny\texttt{one\_for\_one} $|$ \texttt{one\_for\_all} $|$ \texttt{rest\_for\_one}}\\
        {\tiny\texttt{simple\_one\_for\_one}（动态子进程模板）}\\
        {\tiny\textcolor{erlinit}{\textbf{注意：}}\texttt{init/1} 返回 \texttt{\{ok, \{SupFlags, ChildSpecs\}\}}}
    };

    % ---- Right-Bottom: Child Processes (managed by supervisor) ----
    ...
    \node[font=\small\bfseries\color{erlgreen!70!black}, anchor=north west] at ([xshift=3pt, yshift=-3pt]rb.north west) {子进程（由supervisor管理）};
    \node[font=\scriptsize, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rb.north west) {
        \textcolor{erlred!80!black}{\texttt{start\_link(\textcolor{erlmodule}{\textbf{?MODULE}}, Args)}} 之后：\\[3pt]
        \textcolor{erlblue}{\ding{72}} \texttt{child\_1:} \textcolor{erlblue}{\textbf{gen\_server}} \texttt{\{M1,F,A\}} $\rightarrow$ \texttt{Pid\_1}\\
        \textcolor{erlgreen!70!black}{\ding{72}} \texttt{child\_2:} \textcolor{erlgreen!70!black}{\textbf{gen\_event}} \texttt{\{M2,F,A\}} $\rightarrow$ \texttt{Pid\_2}\\
        \textcolor{erlpurple}{\ding{72}} \texttt{child\_3:} \textcolor{erlpurple}{\textbf{gen\_statem}} \texttt{\{M3,F,A\}} $\rightarrow$ \texttt{Pid\_3}\\[2pt]
        {\tiny\textcolor{gray}{子进程可以是任意 OTP behaviour（gen\_server /}}\\
        {\tiny\textcolor{gray}{gen\_event / gen\_statem），也可以是 supervisor}}
    };
    % ==== Arrows ====

    % ==== Phase 1: Startup ====

    % Arrow ①: start_link — the KEY arrow, from Callback Module (lt) to Generic Supervisor (rt)
    \draw[-{Stealth[length=6pt]}, ultra thick, erlred!80!black, line width=1.6pt]
        ...
        {\textcolor{black}{\ding{172}} 启动监督进程 $\rightarrow$ 传递模块名\\[1pt]
         \texttt{start\_link(\textcolor{erlmodule}{\textbf{?MODULE}}, Args)}} ([yshift=3.8mm]rt.west);

    % Arrow ③: Return {ok, Pid} — from Generic Supervisor back to Callback Module
    \draw[-{Stealth[length=6pt]}, ultra thick, erlpid!90!black, line width=1.6pt]
        ...
        {\textcolor{black}{\ding{174}} 返回 \texttt{\textcolor{erlpid}{\textbf{\{ok, Pid\}}}}\\[1pt]
         {\tiny \textcolor{erlinit}{\textbf{init/1}} 成功后启动所有子进程，返回 Pid}} ([yshift=-6.5mm]lt.east);

    % ==== ChildSpec Info Box (center-bottom gap) ====
    \node[draw=erlyellow!80!black, fill=erlyellow!10, rounded corners=5pt,
          minimum width=4.0cm, minimum height=2.8cm, text width=3.6cm, align=left,
          font=\tiny] (childspecbox) at (0.8, -3.45) {
        \textcolor{erlyellow!70!black}{\scriptsize\bfseries ChildSpec（子进程规格）}\\[2pt]
        \texttt{\#\{}\\
        \hspace{4pt}\textcolor{erlblue}{\textbf{id}} \texttt{=>} \textit{child\_id}\texttt{,}\\
        \hspace{4pt}\textcolor{erlred}{\textbf{start}} \texttt{=>} \texttt{\{M, F, A\}}\texttt{,}\\
        \hspace{4pt}\textcolor{erlgreen}{\textbf{restart}} \texttt{=>}\\
        \hspace{12pt}\texttt{permanent|transient|temporary}\texttt{,}\\
        \hspace{4pt}\textcolor{erlpurple}{\textbf{shutdown}} \texttt{=>} \texttt{brutal\_kill|Timeout}\texttt{,}\\
        \hspace{4pt}\textcolor{erlinit}{\textbf{type}} \texttt{=>} \texttt{worker | supervisor}\texttt{,}\\
        \hspace{4pt}\textcolor{gray}{\textbf{modules}} \texttt{=>} \texttt{[Module] | dynamic}\\
        \texttt{\}}
    };

    % ==== Phase 2: Runtime ====

    % Arrow ④: Client API (lb) -> Callback Module (lt) (manage children)
    \draw[-{Stealth[length=5pt]}, thick, erlblue, line width=1.2pt]
        ([xshift=-4mm]lb.north) -- node[left, font=\scriptsize, align=center, anchor=north east, yshift=6pt] {\textcolor{black}{\ding{175}} \textcolor{erlblue}{start\_child /}\\[1pt]\textcolor{erlblue}{which\_children}} ([xshift=-4mm]lt.south);

    % Arrow ②: Generic Supervisor -> Child Processes (start children)
    \draw[-{Stealth[length=6pt]}, thick, erlgreen!70!black, line width=1.2pt]
        ...
        {\scriptsize\bfseries \textcolor{black}{\ding{173}} 启动子进程\\[1pt]
         \tiny 按 ChildSpec 顺序调用 \texttt{\{M,F,A\}}} ([xshift=0mm]rb.north);
    ...
    \node[font=\tiny\color{erlred!70!black}, anchor=north west, text width=5.8cm, align=left] at ([xshift=6pt, yshift=-10pt]lb.south west)
        {\ding{72} \texttt{start\_link} 是连接左右两半的关键桥梁：\\
         \ding{72} \texttt{\textcolor{erlmodule}{\textbf{?MODULE}}} 使得 supervisor 知道调用哪个模块的 \texttt{init/1}\\
         \ding{72} 这里 \texttt{\textcolor{erlmodule}{\textbf{?MODULE}}} 是宏，指的是 \texttt{worker\_sup}\\
         \ding{72} supervisor 只有一个回调函数 \texttt{init/1}};
    ...
    \node[font=\tiny\color{erlred!70!black}, anchor=north west, text width=5.8cm, align=left] at ([xshift=6pt, yshift=-10pt]rb.south west)
        {\ding{72} \textcolor{erlinit}{\textbf{\texttt{Mod:init/1}}} 启动时调用，返回 \texttt{\{ok, \{SupFlags, ChildSpecs\}\}}\\
         \ding{72} \texttt{SupFlags} 定义重启策略、强度和周期\\
         \ding{72} \texttt{ChildSpecs} 列表中的子进程按顺序启动\\
         \ding{72} 子进程可以是 \textcolor{erlblue}{\textbf{gen\_server}} / \textcolor{erlgreen!70!black}{\textbf{gen\_event}} / \textcolor{erlpurple}{\textbf{gen\_statem}} / \textbf{supervisor}\\
         \ding{72} 启动成功后返回 \textcolor{erlpid}{\textbf{\texttt{\{ok, Pid\}}}}，Pid 即监督进程标识};
\end{tikzpicture}
