...
\wideimgslide{第二张图：rebar3 把启动入口包装在配置里}{02-rebar3-config-dependence.png}{
...
\wideimgslide{第三张图：compile 阶段已经在分析 application}{03-rebar3-comple.png}{
  \begin{minipage}{0.92\linewidth}
    \begin{block}{截图解析}
      \begin{itemize}
...
\wideimgslide{第四张图：真正入口在 .app.src 的 mod 字段}{04-rebar3-app.src.png}{
  \begin{minipage}{0.92\linewidth}
    \begin{columns}[T,onlytextwidth]
...
\wideimgslide{第五张图：shell 启动后，full\_otp\_app 已经在列表里}{05-application-lists-command.png}{
  \begin{minipage}{0.92\linewidth}
    \begin{block}{截图解析}
      \begin{itemize}
        \item \code{rebar3 shell --sname app\_demo} 启动节点。
...
\wideimgslide{第六张图：Observer 把启动结果画成 supervision tree}{06-observer-app.png}{
  \begin{minipage}{0.92\linewidth}
    \begin{columns}[T,onlytextwidth]
...
\wideimgslide{第七张图：application 可以被 stop 管理}{07-app-stop.png}{
  \begin{minipage}{0.92\linewidth}
    \begin{block}{截图解析}
      \begin{itemize}
...
\wideimgslide{第八张图：stop 之后，Observer 中 app 消失}{08-observer-app-stop.png}{
  \begin{minipage}{0.92\linewidth}
...
\wideimgslide{第九张图：application:start 重新建立进程树}{09-observer-app-start.png}{
  \begin{minipage}{0.92\linewidth}
    \begin{columns}[T,onlytextwidth]
      \column{0.50\textwidth}
      \begin{block}{shell 中重新启动}
        \code{application:start(full\_otp\_app).} 触发 callback module，再次启动 top supervisor 和 workers。
      \end{block}
      \column{0.46\textwidth}
      \begin{exampleblock}{日志再次出现}
        \code{kv\_server}、\code{event\_bus}、\code{traffic\_light}、\code{data\_store}、\code{data\_cache} 又被初始化。
      \end{exampleblock}
    \end{columns}
  \end{minipage}
}

\imgslide{第十张图：application 模块的入口很集中}{10-application-command-list.png}{
  \begin{block}{截图解析}
    shell 自动补全展示了 \code{application:} 模块的一组函数。
  \end{block}
