...
\end{lstlisting}
    ...
    \begin{alertblock}{这一集的讲法}
      \begin{itemize}
        ...
        \item 再看初始化后的目录：此时先理解工程骨架，不急着背配置字段。
        \item 然后运行 \code{rebar3 compile}，再进入 \code{full\_otp\_app.app.src} 解释 OTP 入口。
      \end{itemize}
    \end{alertblock}
  \end{columns}
  \vspace{0.2cm}
...
\begin{frame}[fragile]{第一张图：初始化后的工程骨架}
  \begin{columns}[T,onlytextwidth]
    \column{0.54\textwidth}
    \begin{block}{01-rebar3-app-init.png 文字化}
\begin{lstlisting}[
  basicstyle=\ttfamily\tiny\color{terminalfg},
  backgroundcolor=\color{terminalbg},
  frame=single,
...
\end{lstlisting}
    ...
    \footnotesize
    \begin{block}{这一页先看骨架，不急着解释字段}
      \begin{itemize}
        \item 根目录里的 \code{rebar.config} 是工具入口。
        \item \code{src/} 是 OTP 代码入口：resource file、callback、supervisor、workers 都在这里。
        \item 这一步只建立方向感：工程已经按 OTP application 的方式组织好了。
      \end{itemize}
    \end{block}
    \vspace{0.15cm}
    \begin{exampleblock}{承接下一页}
      接下来运行 \code{rebar3 compile}，工具会按 application 维度分析、编译并生成可被 VM 使用的 application 描述。
    \end{exampleblock}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{运行 rebar3 compile：工具开始按 application 处理工程}
  \begin{columns}[T,onlytextwidth]
...
\end{lstlisting}
    ...
    \begin{block}{这里发生了什么}
      \begin{itemize}
        \item rebar3 先处理 \code{deps} 中声明的依赖。
        \item \code{Analyzing applications...} 表明它按 OTP application 维度分析工程。
        \item 编译后，\code{.app.src} 中的信息会进入构建产物，成为 VM 能使用的 application 描述。
      \end{itemize}
    \end{block}
    \vspace{0.15cm}
    \centering
    \good{现在再看 \code{full\_otp\_app.app.src}，它就不是孤立文件，而是工具链要处理的 OTP 入口。}
  \end{columns}
\end{frame}
