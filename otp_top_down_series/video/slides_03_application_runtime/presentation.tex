...
\title{Application（1）runtime application}
...
at ([yshift=1.75cm]current page.center) {Application（1）runtime application};
...
% ── Concepts ───────────────────────────────────────────────────────
\begin{frame}{先借用一个熟悉的类比：Application 像"可被运行时管理的包"}
  \small
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
    \begin{block}{其他生态里常见的"包"}
      \begin{itemize}
        \item \keypoint{Java}：\code{jar} / \code{war} 把 class、资源和元信息打包。
        \item \keypoint{Linux}：\code{rpm} / \code{deb} 描述文件、依赖、安装脚本和版本。
        \item \keypoint{Python}：package / wheel 把 module、依赖和入口组织起来。
      \end{itemize}
    \end{block}
    \column{0.48\textwidth}
    \begin{exampleblock}{Erlang/OTP 里的 Application}
      \begin{itemize}
        \item 也是一个有名字、有版本、有依赖的组织单元。
        \item 不只是代码包，还可以声明如何被 VM 启动和停止。
        \item runtime application 还会把 supervisor tree 作为运行时结构带起来。
      \end{itemize}
    \end{exampleblock}
  \end{columns}
  \vspace{0.25cm}
\begin{alertblock}{这一集先整体认识 Application}
    可以先把 Erlang Application 理解成：\keypoint{被 BEAM VM 识别、加载、启动、停止和观察的 OTP 包单元}。
  \end{alertblock}
\end{frame}

\begin{frame}{Erlang的 Application 先分成两种}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
...
\end{frame}
\begin{frame}[fragile]{一条主线：从工具的配置配置文件，到 OTP 的启动入口}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
...
\end{lstlisting}
  ...
  \vspace{0.2cm}
  \centering
  \warn{配置字段不用背诵，只需理解，有专门的构建工具来生成}
\end{frame}
...
\begin{frame}[fragile]{运行 rebar3 compile：依赖下载之后生成 application 描述文件 app.src}
  \begin{columns}[T,onlytextwidth]
...
\begin{frame}[fragile]{compile 之后再看目录：app.src 和构建产物出现了}
  \begin{columns}[T,onlytextwidth]
    \column{0.58\textwidth}
    \begin{block}{编译后的目录结构}
\begin{lstlisting}[
  basicstyle=\ttfamily\tiny\color{terminalfg},
  backgroundcolor=\color{terminalbg},
  frame=single,
...
[ericksun@ERICKSUN-MC1:full_otp_app] (*@\textcolor{warning}{(main)}@*) (*@\textcolor{accent}{\$}@*) tree -L 2
(*@\textcolor{erlblue}{.}@*)
|-- README.txt
|-- (*@\textcolor{erlblue}{\_build}@*)
|   `-- (*@\textcolor{erlblue}{default}@*)
|-- (*@\textcolor{erlblue}{data}@*)
|   `-- full_otp_data_store.dets
|-- rebar.config
|-- rebar.lock
`-- (*@\textcolor{erlblue}{src}@*)
    |-- data_cache.erl
...
\end{lstlisting}
    ...
    \begin{block}{这一页看变化}
      \begin{itemize}
        \item \code{\_build/default} 是 rebar3 编译后产生的构建目录。
        \item \code{rebar.lock} 记录依赖解析结果，让后续构建可重复。
        \item \code{src/full\_otp\_app.app.src} 现在出现了，下一页就从它进入 OTP application 的入口。
      \end{itemize}
    \end{block}
    \vspace{0.15cm}
    \begin{exampleblock}{自然过渡}
      目录已经从“源码骨架”变成“可被工具识别的 application 工程”，所以接下来再打开 \code{full\_otp\_app.app.src}。
    \end{exampleblock}
  \end{columns}
\end{frame}
...
\begin{frame}{配置很多，但先抓住三个有用字段}
  \vspace{-0.2cm}
  \begin{center}
    \textit{\large Erlang 的 Application，\keypoint{实现复杂}，但 \good{使用很简单}。}
  \end{center}
  \vspace{0.1cm}
  \begin{columns}[T,onlytextwidth]
  ...
  \vspace{0.3cm}
  \centering
  \Large \keypoint{这些字段不用记忆，全部是由构建工具自动生成}
\end{frame}
...
\end{lstlisting}
    ...
    \end{exampleblock}
    \vspace{0.15cm}
    \begin{alertblock}{使用起来就是这么简单}
      系统只要求你提供一个 \code{start/2}，把 supervisor 启动起来；application 的启动边界就完成了。
    \end{alertblock}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{启动 BEAM 进程，再用命令查看 Application 的运行状态}
  \begin{columns}[T,onlytextwidth]
    \column{0.60\textwidth}
