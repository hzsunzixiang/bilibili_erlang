...
\begin{frame}[fragile]{这一集只抓一条主线：配置如何变成进程树}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
...
\end{lstlisting}
    ...
    \begin{alertblock}{这一集的讲法}
      \begin{itemize}
        \item 先看 \code{full\_otp\_app.app.src}，因为它是 application 的声明入口。
        \item 再看工具链如何自动生成、编译、启动这个入口。
        \item 最后用 shell 和 Observer 验证：配置确实变成了运行中的进程树。
      \end{itemize}
    \end{alertblock}
  \end{columns}
  \vspace{0.2cm}
  \centering
  \warn{目标不是背 application controller 内部实现，而是建立“配置 -> 启动 -> 进程树”的整体图景。}
\end{frame}

\begin{frame}[fragile]{先看核心配置：full\_otp\_app.app.src}
  \begin{columns}[T,onlytextwidth]
...
\end{lstlisting}
    ...
    \footnotesize
    \begin{block}{先给读者一个全貌}
      \begin{itemize}
        \item 这个文件描述一个 OTP application：名字、版本、依赖、启动入口等。
        \item 后面会逐个解释这些配置项如何影响运行时行为。
        \item 入门阶段不需要掌握所有字段，只需要抓住几个最常用、最有解释力的字段。
      \end{itemize}
    \end{block}
    \vspace{0.15cm}
    \begin{alertblock}{工具链会替你做大部分事情}
      \code{rebar3}、\code{erlang.mk} 等构建工具通常会自动生成、校验和打包这类配置。我们看它，是为了理解现象，不是为了手写所有细节。
    \end{alertblock}
  \end{columns}
\end{frame}
...
\begin{frame}[fragile]{工具生成的工程骨架，已经把 OTP 入口放好了}
  \begin{columns}[T,onlytextwidth]
...
`-- (*@\textcolor{erlblue}{src}@*)
    ...
    |-- (*@\textcolor{warning}{full_otp_app.app.src}@*)
    |-- (*@\textcolor{accent}{full_otp_app_app.erl}@*)
    |-- (*@\textcolor{accent}{full_otp_app_sup.erl}@*)
    |-- kv_server.erl
    `-- traffic_light.erl
...
\end{lstlisting}
    ...
    \footnotesize
    \begin{block}{从目录读出三层入口}
      \begin{itemize}
        \item \code{full\_otp\_app.app.src}: 声明 application 是什么、依赖什么、从哪里启动。
        \item \code{full\_otp\_app\_app.erl}: application callback module，承接 \code{mod} 字段。
        \item \code{full\_otp\_app\_sup.erl}: top supervisor，启动后真正管理长期进程。
      \end{itemize}
    \end{block}
    \vspace{0.15cm}
    \begin{exampleblock}{连贯理解}
      目录不是孤立截图，它是在告诉我们：工具已经把 OTP application 的标准入口组织好了。
    \end{exampleblock}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{rebar3 继续把依赖和启动行为收进配置}
  \begin{columns}[T,onlytextwidth]
...
{(*@\textcolor{erlblue}{erl_opts}@*), [debug_info]}.
{(*@\textcolor{erlblue}{deps}@*), [
    {(*@\textcolor{accent}{observer_cli}@*), "1.8.8"},
    {(*@\textcolor{accent}{recon}@*), "2.5.6"}
]}.
{(*@\textcolor{erlblue}{shell}@*), [
    {(*@\textcolor{accent}{apps}@*), [(*@\textcolor{accent}{full_otp_app}@*)]}
]}.
\end{lstlisting}
    \end{block}
    \column{0.46\textwidth}
    \footnotesize
    \begin{block}{这里不是另一条线，而是在服务同一个目标}
      \begin{itemize}
        \item \code{deps} 让工具自动拉取、编译依赖 application。
        \item \code{shell} 里的 \code{apps} 让进入 shell 时自动启动 \code{full\_otp\_app}。
        \item 对使用者来说，入口仍然很集中：配置文件 + 少量命令。
      \end{itemize}
    \end{block}
    \vspace{0.15cm}
    \begin{alertblock}{为什么说 application 用起来简单}
      复杂的生成、依赖、启动顺序被工具链包装；我们只需要知道关键配置在哪里。
    \end{alertblock}
  \end{columns}
\end{frame}

\begin{frame}[fragile]{编译时，rebar3 已经按 application 组织工程}
  \begin{columns}[T,onlytextwidth]
    \column{0.56\textwidth}
...
(*@\textcolor{warning}{Fetching observer_cli v1.8.8}@*)
(*@\textcolor{warning}{Fetching recon v2.5.6}@*)
(*@\textcolor{erlblue}{Analyzing applications...}@*)
(*@\textcolor{warning}{Compiling recon}@*)
(*@\textcolor{warning}{Compiling observer_cli}@*)
===> (*@\textcolor{erlblue}{Analyzing applications...}@*)
(*@\textcolor{warning}{Compiling full_otp_app}@*)
\end{lstlisting}
    \end{block}
    \column{0.40\textwidth}
    \footnotesize
    \begin{block}{这一页证明工具链理解 application}
      \begin{itemize}
        \item 先解析依赖，再编译依赖 application。
        \item \code{Analyzing applications...} 表明 application 是构建单元。
        \item 最后编译当前工程 \code{full\_otp\_app}。
      \end{itemize}
    \end{block}
    \vspace{0.15cm}
    \centering
    \good{到这里，配置已经从“文本文件”进入工具链的处理流程。}
  \end{columns}
\end{frame}
...
[ericksun@ERICKSUN-MC1:full_otp_app] (main) $ (*@\textcolor{accent}{rebar3 shell --sname app_demo}@*)
(*@\textcolor{erlblue}{Verifying dependencies...}@*)
(*@\textcolor{erlblue}{Analyzing applications...}@*)
(*@\textcolor{warning}{Compiling full_otp_app}@*)
...
(*@\textcolor{accent}{[full_otp_app] starting...}@*)
(*@\textcolor{accent}{[kv_server] init: pid=<0.258.0>}@*)
(*@\textcolor{accent}{[event_bus_handler] init}@*)
(*@\textcolor{accent}{[event_bus] started with default handler}@*)
(*@\textcolor{accent}{[traffic_light] init: starting at red}@*)
(*@\textcolor{accent}{[data_store] init: pid=<0.262.0>}@*)
(*@\textcolor{accent}{[data_cache] init: ETS cache table created}@*)
===> (*@\textcolor{accent}{Booted full_otp_app}@*)
(app_demo@ERICKSUN-MC1)1> (*@\textcolor{primary}{application:which_applications().}@*)
[{(*@\textcolor{erlblue}{full_otp_app}@*), "...gen_server, gen_event and gen_statem", "0.1.0"},
...
(app_demo@ERICKSUN-MC1)3> (*@\textcolor{primary}{application:stop(full_otp_app).}@*)
(*@\textcolor{accent}{[event_bus_handler] terminate: reason=stop}@*)
(*@\textcolor{muted}{[full_otp_app] stopped.}@*)
=INFO REPORT==== ... ===
...
(app_demo@ERICKSUN-MC1)3> (*@\textcolor{primary}{application:which_applications().}@*)
[{(*@\textcolor{erlblue}{observer_cli}@*), "...", "1.8.8"},
 {(*@\textcolor{erlblue}{recon}@*), "Diagnostic tools ...", "2.5.6"},
 {stdlib, "ERTS  CXC 138 10", "5.2.3.5"},
 {(*@\textcolor{primary}{kernel}@*), "ERTS  CXC 138 10", "9.2.4.10"}]

(app_demo@ERICKSUN-MC1)4> (*@\textcolor{primary}{application:start(full_otp_app).}@*)
(*@\textcolor{accent}{[full_otp_app] starting...}@*)
(*@\textcolor{accent}{[kv_server] init: pid=<0.317.0>}@*)
(*@\textcolor{accent}{[event_bus_handler] init}@*)
(*@\textcolor{accent}{[event_bus] started with default handler}@*)
(*@\textcolor{accent}{[traffic_light] init: starting at red}@*)
(*@\textcolor{accent}{[data_store] init: pid=<0.321.0>}@*)
(*@\textcolor{accent}{[data_cache] init: ETS cache table created}@*)
(*@\textcolor{accent}{ok}@*)
\end{lstlisting}
    \end{block}
...
takeover/2           unload/1
unset_env/2,3        (*@\textcolor{accent}{which_applications/0}@*)
\end{lstlisting}
    \end{block}
    \column{0.42\textwidth}
    \footnotesize
