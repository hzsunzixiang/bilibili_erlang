...
\begin{frame}[fragile]{先建立工程骨架：代码从哪里进入 OTP}
  \begin{columns}[T,onlytextwidth]
    \column{0.54\textwidth}
    \begin{block}{初始化后的目录结构}
\begin{lstlisting}[
  basicstyle=\ttfamily\tiny\color{terminalfg},
  backgroundcolor=\color{terminalbg},
  frame=single,
...
\end{lstlisting}
    ...
    \begin{block}{这一步只建立方向感}
      \begin{itemize}
        ...
        \item \code{src/} 里放 application resource、callback、supervisor 和 worker 模块。
        \item 先看目录，是为了让后面的 \code{compile} 和 \code{app.src} 不显得突兀。
      \end{itemize}
    \end{block}
    \vspace{0.15cm}
