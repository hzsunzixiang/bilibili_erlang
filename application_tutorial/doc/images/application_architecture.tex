...
\begin{tikzpicture}[scale=0.68, every node/.style={scale=0.68},
    ...
    \node[font=\scriptsize, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rb.north west) {
        ...
        \hspace{12pt}\textcolor{erlgreen!70!black}{\ding{72}} \texttt{child\_1:} \textcolor{erlgreen!70!black}{\textbf{gen\_server}} $\rightarrow$ \texttt{frequency}\\
        \hspace{12pt}\textcolor{erlpurple}{\ding{72}} \texttt{child\_2:} \textcolor{erlpurple}{\textbf{gen\_event}} $\rightarrow$ \texttt{freq\_overload}\\
        \hspace{12pt}\textcolor{erlinit}{\ding{72}} \texttt{child\_3:} \textcolor{erlinit}{\textbf{gen\_statem}} $\rightarrow$ \texttt{phone\_fsm}\\[2pt]
        {\tiny\textcolor{gray}{Top Supervisor 终止 → Application Master 检测 → Application 终止}}
    };
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlpid!90!black, line width=1.6pt]
        ...
        {\textcolor{black}{\ding{174}} 返回 \texttt{\textcolor{erlpid}{\textbf{\{ok, Pid\}}}}\\[1pt]
         {\tiny \textcolor{erlmodule}{\textbf{Module:start/2}} 成功后，Application 进入运行状态}} ([yshift=-6.5mm]lt.east);
    ...
    \draw[-{Stealth[length=6pt]}, thick, erlgreen!70!black, line width=1.2pt]
        ...
        {\scriptsize\bfseries \textcolor{black}{\ding{173}} 启动监督树\\[1pt]
         \tiny Module:start/2 $\rightarrow$ bsc\_sup:start\_link()} ([xshift=0mm]rb.north);
    ...
    \node[font=\tiny\color{erlred!70!black}, anchor=north west, text width=5.8cm, align=left] at ([xshift=6pt, yshift=-10pt]rb.south west)
        {\ding{72} Application Controller 加载 \texttt{.app} 文件，检查 \texttt{applications} 依赖\\
         \ding{72} Application Master 调用 \textcolor{erlmodule}{\textbf{\texttt{Module:start/2}}} 启动监督树\\
