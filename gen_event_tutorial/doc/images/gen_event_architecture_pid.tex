...
\begin{tikzpicture}[scale=0.68, every node/.style={scale=0.68},
    ...
    \node[draw=erlpurple!60, fill=erlpurple!3, rounded corners=6pt,
          minimum width=6.6cm, minimum height=13.0cm] (leftframe) at (-4.8, -0.55) {};
...
\node[font=\small\ttfamily\bfseries\itshape\color{black}, anchor=north west] at ([xshift=3pt, yshift=-1pt]leftframe.north west) {event\_counter.erl};
    ...
    \node[draw=erlblue!60, fill=erlblue!3, rounded corners=6pt,
          minimum width=6.6cm, minimum height=13.0cm] (rightframe) at (6.4, -0.55) {};
...
\node[font=\small\ttfamily\bfseries\itshape\color{black}, anchor=north west] at ([xshift=3pt, yshift=-1pt]rightframe.north west) {gen\_event.erl};
    ...
    \node[draw=erlgreen, fill=erlgreen!8, rounded corners=5pt,
          minimum width=6.2cm, minimum height=6.8cm, text width=5.8cm, align=left] (lt) at (-4.8, 2.05) {};
\node[font=\small\bfseries\color{erlgreen!80!black}, anchor=north west] at ([xshift=3pt, yshift=-3pt]lt.north west) {Manager 进程（启动gen\_event）};
    ...
    \node[draw=erlred!70, fill=erlred!8, rounded corners=3pt,
        ...
        {\textcolor{erlred!80!black}{\ding{72} \{ok, Pid\} = gen\_event:start\_link()}};
    \node[draw=erlgreen!70, fill=erlgreen!8, rounded corners=3pt,
          font=\scriptsize\ttfamily\bfseries, inner sep=3pt, anchor=north]
        (addhandler) at ([yshift=-58pt]lt.north)
        {\textcolor{erlgreen!80!black}{\ding{72} gen\_event:add\_handler(Pid, \textcolor{erlmodule}{\textbf{?MODULE}}, Args)}};
    \node[font=\scriptsize\ttfamily, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-78pt]lt.north west) {
\textcolor{erlinit}{\textbf{init/1}}\\
    ...
    \node[font=\tiny\color{gray}, anchor=south west] at ([xshift=6pt, yshift=3pt]lb.south west)
        {备注：\texttt{\textcolor{erlpid}{\textbf{MgrRef}}} = \texttt{pid() | \{local, Name\}}};

    % ---- Right-Top: Generic Server ----
    \node[draw=erlblue, fill=erlblue!10, rounded corners=5pt,
          minimum width=6.2cm, minimum height=6.8cm, text width=5.8cm, align=left] (rt) at (6.4, 2.05) {};
\node[font=\small\bfseries\color{erlblue}, anchor=north west] at ([xshift=3pt, yshift=-3pt]rt.north west) {Generic Event Manager Library};
    \node[font=\scriptsize, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rt.north west) {
        ...
        {\tiny\texttt{start\_link} $\rightarrow$ \texttt{proc\_lib:start\_link} $\rightarrow$ \texttt{init\_it} $\rightarrow$ 空 Handler 列表}\\[6pt]
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlred!80!black, line width=1.6pt]
        ([yshift=18mm]lt.east) -- node[above, font=\scriptsize, align=center, text=erlred!80!black]
        {\textcolor{black}{\ding{172}} 启动Manager\\[1pt]
         \texttt{gen\_event:start\_link()}} ([yshift=18mm]rt.west);
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlpid!90!black, line width=1.6pt]
        ([yshift=8mm]rt.west) -- node[below, font=\scriptsize, align=center, text=erlpid!80!black]
        {\textcolor{black}{\ding{173}} 返回 \texttt{\textcolor{erlpid}{\textbf{\{ok, Pid\}}}}\\[1pt]
         {\tiny Manager 进程启动成功，返回进程标识符}} ([yshift=8mm]lt.east);

    % ==== Phase 2: Register Handler ====
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlgreen!80!black, line width=1.6pt]
        ([yshift=-4mm]lt.east) -- node[above, font=\scriptsize, align=center, text=erlgreen!80!black]
        {\textcolor{black}{\ding{174}} 注册Handler $\rightarrow$ 传递模块名\\[1pt]
         \texttt{add\_handler(Pid, \textcolor{erlmodule}{\textbf{?MODULE}}, Args)}} ([yshift=-4mm]rt.west);

    % Arrow ⑤: Return ok — from Generic Server back to Callback Module
    \draw[-{Stealth[length=6pt]}, ultra thick, erlpid!90!black, line width=1.2pt]
        ([yshift=-14mm]rt.west) -- node[below, font=\scriptsize, align=center, text=erlpid!80!black]
        {\textcolor{black}{\ding{176}} 返回 \texttt{\textcolor{erlpid}{\textbf{ok}}}\\[1pt]
         {\tiny \textcolor{erlinit}{\textbf{init/1}} 成功后 Handler 加入 Manager 列表}} ([yshift=-14mm]lt.east);

    % ==== Phase 3: Runtime ====

    % Arrow ⑥: Client API (lb) -> Callback Module (lt) (notify / call)
    ...
    \node[font=\tiny\color{erlred!70!black}, anchor=north west, text width=5.8cm, align=left] at ([xshift=6pt, yshift=-10pt]lb.south west)
        {\ding{72} \texttt{add\_handler} 是连接左右两半的关键桥梁：\\
