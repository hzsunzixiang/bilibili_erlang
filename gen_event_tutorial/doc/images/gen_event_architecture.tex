...
\begin{tikzpicture}[scale=0.68, every node/.style={scale=0.68},
    ...
    \node[font=\small\bfseries\color{erlpurple}, anchor=south] at (leftframe.north) {用户定义模块（Server的启动）};
...
\node[font=\small\bfseries\color{erlgreen!80!black}, anchor=north west] at ([xshift=3pt, yshift=-3pt]lt.north west) {manager进程（启动gen\_event）};
...
\textcolor{erlinit}{\textbf{init/1}}\\
    ...
    \node[font=\scriptsize\ttfamily, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-18pt]lb.north west) {
        gen\_server:call(\textcolor{erlpid}{\textbf{ServerRef}}, Req)\\
        gen\_server:cast(\textcolor{erlpid}{\textbf{ServerRef}}, Msg)\\
        gen\_server:send\_request(\textcolor{erlpid}{\textbf{ServerRef}}, Req)\\
        gen\_server:stop(\textcolor{erlpid}{\textbf{ServerRef}})
    };
    % Note: ?MODULE = counter_server
    \node[font=\tiny\color{gray}, anchor=south west] at ([xshift=6pt, yshift=3pt]lb.south west)
        {备注：\texttt{\textcolor{erlpid}{\textbf{ServerRef}}} = \texttt{pid()}};
    ...
    \node[font=\scriptsize, text width=5.4cm, align=left, anchor=north west] at ([xshift=6pt, yshift=-16pt]rt.north west) {
        ...
        {\tiny\texttt{parent}\,=\,父进程Pid,\; \texttt{module}\,=\,\textcolor{erlmodule}{\textbf{\texttt{?MODULE}}}}\\
        {\tiny\texttt{handle\_call}\,=\,\texttt{fun Mod:handle\_call/3}}        {\tiny\texttt{handle\_cast}\,=\,\texttt{fun Mod:handle\_cast/2},\;\ldots}\\
    ...
    \draw[-{Stealth[length=6pt]}, ultra thick, erlpid!90!black, line width=1.6pt]
        ...
        {\textcolor{black}{\ding{174}} 返回 \texttt{\textcolor{erlpid}{\textbf{\{ok, Pid\}}}}\\[1pt]
         {\tiny \textcolor{erlinit}{\textbf{init/1}} 成功后通过 \texttt{init\_ack} 返回 Pid}} ([yshift=-6.5mm]lt.east);
