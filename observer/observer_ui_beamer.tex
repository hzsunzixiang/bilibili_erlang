...
\imgslide{打开Observer 后的第一印象}{System.png}{%
  \begin{block}{打开后先看到 BEAM node}
    \begin{itemize}
    ...
    \end{itemize}
  \end{block}
  \begin{alertblock}{先建立Observe的直观印象}
    这一页是 Observer 打开后就能看到的系统总览：先看见 BEAM node，再进入后面的具体页面。
  \end{alertblock}
}
...
\imgslide{Processes：先看见 Erlang process}{Applications_process_list.png}{%
  \begin{block}{进程为什么重要？}
    ...
    \end{itemize}
  \end{block}
  \begin{exampleblock}{先做进程整体讲解}
    先知道 Observer 能看到 process，再继续看 process 内部现场和 process 背后的表数据。
  \end{exampleblock}
}
...
% ── Start ──────────────────────────────────────────────────────────
\begin{frame}{从进程视角进入 OTP 应用观察}
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
    \begin{block}{前面已经建立的视角}
      \begin{itemize}
        \item Observer 可以直接看到 Erlang process
        \item process 有 mailbox、state、stack trace
        \item worker process 背后还可能关联 ETS / DETS
      \end{itemize}
    \end{block}
    \column{0.46\textwidth}
    \begin{exampleblock}{现在正式开始}
      接下来启动一个完整 OTP demo application，正式介绍 Observer 的启动方式，以及它如何应用到 application / supervisor / worker process 的观察上。
    \end{exampleblock}
  \end{columns}
\end{frame}
...
% ── Process deep dive ──────────────────────────────────────────────
\begin{frame}{进程相关部分：从 worker process 继续展开}
  \begin{columns}[T,onlytextwidth]
    \column{0.48\textwidth}
    \begin{block}{进程内部}
      \begin{itemize}
  ...
  \vspace{0.2cm}
  \centering
  \good{Applications 追到 worker process 后，不只看进程身份，还要继续看进程状态、消息和它背后的表数据。}
\end{frame}
...
% ── Demo flow ──────────────────────────────────────────────────────
\begin{frame}{推荐课堂演示顺序：先进程整体，再展开 Applications}
  \small
  \begin{enumerate}
    ...
    \item 切到 \code{Processes}，先建立 Erlang process 级别观察
    \item 继续看 \code{Messages} / \code{Dictionary} / \code{Stack Trace}，深入进程现场
    \item 打开 \code{Table Viewer}，观察 worker process 背后的 ETS / DETS 数据
    \item 再切到 \code{Applications}，从 application 展开 OTP 运行时结构
...
\begin{frame}{小结：Observer UI 的真正意义}
  \begin{block}{它让 Erlang/OTP 的核心抽象可见}
    \begin{itemize}
      ...
      \item Messages / Dictionary / Stack Trace 是继续深入进程现场
      \item Table Viewer 能接着观察 worker process 背后的 ETS / DETS 数据
    \end{itemize}
  \end{block}
