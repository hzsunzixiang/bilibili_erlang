...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    ...
    \node[anchor=south, font=\scriptsize, text=dark!45]
      ...
      {故事线：Process \enspace|\enspace Observer \enspace|\enspace 为什么需要 OTP \enspace|\enspace Applications};
  \end{tikzpicture}
\end{frame}

% ── Process first ──────────────────────────────────────────────────
\begin{frame}{今天先不讲 OTP}
  \begin{columns}[T,onlytextwidth]
    \column{0.52\textwidth}
    \begin{alertblock}{先认识 Erlang 最核心的抽象}
      Erlang 最大的特色，不是语法，不是模式匹配，而是 \keypoint{Erlang Process}。
    \end{alertblock}
    \begin{block}{为什么它重要？}
      \begin{itemize}
...
\begin{frame}{一个 Erlang VM 里可以有很多 Process}
  ...
  \vspace{0.25cm}
  \good{一个 Erlang VM 内部，可以同时运行大量 Erlang Process。}
\end{frame}
